...
// 获取声道布局
    ...
    if (af->frame->format != is->audio_param_src.fmt ||
        dec_channel_layout.nb_channels != is->audio_param_src.channel_layout.nb_channels ||
        af->frame->sample_rate != is->audio_param_src.freq)
    {
        swr_free(&is->audio_swr_ctx);
        
        int ret = swr_alloc_set_opts2(&is->audio_swr_ctx,
...
//                                int log_offset, void *log_ctx);
        ...
        if (ret < 0 || swr_init(is->audio_swr_ctx) < 0)
        {
...
//                is->audio_param_tgt.freq, av_get_sample_fmt_name(is->audio_param_tgt.fmt), is->audio_param_tgt.channels);
        ...
        is->audio_param_src.channels = af->frame->ch_layout.nb_channels;
...
static int open_audio_playing(void *arg)
{
    ...
    av_channel_layout_default(&ch_layout, actual_spec.channels);;
    is->audio_param_tgt.channel_layout = ch_layout;
